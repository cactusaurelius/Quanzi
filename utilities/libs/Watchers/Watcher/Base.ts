...
export abstract class Watcher {
   ...
   * @memberof Watcher
   */
  public abstract runWatcher(): Promise<void>;
   ...
   * @memberof Watcher
   */
  public abstract stopWatcher(flushData: boolean): Promise<void>;
      ...
      .catch(async error => {
        ...
        try {
          if (this.restart < 10) {
            ...
            await sleep(30 * 1000);
            logger.info(
              `[Watcher] Try restarting (${this.restart}/10) watcher ${this.id}`
            );
            await this.stop();
            this.run().catch(err => {
              throw err;
            ...
            logger.error(
              `[Watcher] Stopping watcher ${this.id} (can't restart it)`
            );
          }
        } catch (error) {
          throw error;
  ...
  public async save(): Promise<void> {
    ...
    await WatcherModel.findOneAndUpdate({ id: this.id }, watcher, {
      upsert: true,
    });
  }
}
